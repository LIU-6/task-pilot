...
- `--config`：JSON 配置文件
- `--session`：已有 `tmux` 会话名
- `--decision_mode`：`rule` 或 `codex`
- `--max_continue`：最大自动推进轮数
- `--idle_threshold`：输出持续不变多久算 idle
- `--poll_interval`：轮询间隔秒数
- `--history_lines`：每次轮询抓取的 `tmux` 历史行数
- `--rule_continue_prompt`：`rule` 模式使用的固定提示
- `--dry_run`：照常分析，但不向 `tmux` 发送提示
...
- `--dry_run` 模式下，continue 决策只会记录日志，不会真正写回被监控会话。

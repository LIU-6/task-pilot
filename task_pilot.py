...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.idle_threshold <= 0:
        raise RuntimeError("--idle-threshold must be greater than 0")
    if args.max_continue < 0:
        raise RuntimeError("--max-continue must be greater than or equal to 0")
    if args.history_lines <= 0:
        raise RuntimeError("--history-lines must be greater than 0")
    if not args.rule_continue_prompt.strip():
        raise RuntimeError("--rule-continue-prompt must not be empty")
